        ...
        MySQLDatabaseHelper.getAllTransactionsForUser(user_id, getContext(), new MySQLDatabaseHelper.TransactionsCallback() {
            ...
            public void onTransactionsReceived(List<UserCart> transactions) {
                LinearLayout dynamic = view.findViewById(R.id.previousOrder);
                    ...
                    int i = 0;
                    try {
                        for(UserCart transaction: transactions) {
//                System.out.println(i);
                            ...
                            title.setText(transaction.getTimeOrdered());
                            title.setTextSize(30);
                            title.setTextColor(BLACK);
                            //Total formatting
                            TextView priceTotal = new TextView(getContext());
                            priceTotal.setText("$ " + df.format(transaction.getPrice()));
                            ...
                            try{
                                for(int j = 0; j < transaction.getUserCart().size(); j++){
                                    ...
                                    cofName.setText(transaction.getUserCart().get(j).getName());
                                    //Adds the coffee price to the thing
                                    TextView cofPrice = new TextView(getContext());
                                    cofPrice.setText("$ " + df.format(transaction.getUserCart().get(j).getPrice()));
                                    ...
                                    try {
                                        for (int q = 0; q < transaction.getUserCart().get(j).getFlavorItemList().size(); q++) {
                                            ...
                                            flavorName.setText(transaction.getUserCart().get(j).getFlavorItemList().get(q).getName());
                                            //Adds the coffee price to the thing
                                            TextView flavorPrice = new TextView(getContext());
                                            flavorPrice.setText("$ " + transaction.getUserCart().get(j).getFlavorItemList().get(q).getPrice());
                                    ...
                                    try {
                                        for (int q = 0; q < transaction.getUserCart().get(j).getToppingItemList().size(); q++) {
                                            ...
                                            topName.setText(transaction.getUserCart().get(j).getToppingItemList().get(q).getName());
                                            //Adds the coffee price to the thing
                                            TextView topPrice = new TextView(getContext());
                                            topPrice.setText("$ " + df.format(transaction.getUserCart().get(j).getToppingItemList().get(q).getPrice()));
